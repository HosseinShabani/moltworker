...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/openclaw/openclaw-sandbox)
...
| `TELEGRAM_DM_POLICY`        | No       | DM policy: `pairing` (default), `allowlist`, `open`, `disabled`                                            |
| `TELEGRAM_ALLOW_FROM`       | No       | DM allowlist - comma-separated user IDs or usernames (requires `dmPolicy: allowlist`)                      |
| `TELEGRAM_GROUP_POLICY`     | No       | Group policy: `allowlist` (default), `open`, `disabled`                                                    |
| `TELEGRAM_GROUP_ALLOW_FROM` | No       | Group sender allowlist - comma-separated user IDs or usernames                                             |
| `TELEGRAM_REQUIRE_MENTION`  | No       | Require @mention in groups: `true` or `false` (default: false)                                             |
