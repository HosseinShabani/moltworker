 ...
 */
export async function findExistingOpenClawProcess(
  sandbox: Sandbox,
...
export async function ensureOpenClawGateway(
  ...
  if (existingProcess) {
    console.log(
      "Found existing OpenClaw process:",
      existingProcess.id,
      "status:",
    ...
    try {
      console.log(
        "Waiting for OpenClaw gateway on port",
        OPENCLAW_PORT,
        "timeout:",
        STARTUP_TIMEOUT_MS,
      );
  ...
  try {
    console.log(
      "[Gateway] Waiting for OpenClaw gateway to be ready on port",
      OPENCLAW_PORT,
    );
    await process.waitForPort(OPENCLAW_PORT, {
      mode: "tcp",
      timeout: STARTUP_TIMEOUT_MS,
    });
