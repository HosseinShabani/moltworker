...
describe("buildEnvVars", () => {
  ...
  it("includes all Telegram configuration options when set", () => {
    const env = createMockEnv({
      TELEGRAM_BOT_TOKEN: "123:abc",
      TELEGRAM_DM_POLICY: "allowlist",
      TELEGRAM_ALLOW_FROM: "user1,user2",
      TELEGRAM_GROUP_POLICY: "open",
      TELEGRAM_GROUP_ALLOW_FROM: "group_user1",
    ...
    expect(result.TELEGRAM_ALLOW_FROM).toBe("user1,user2");
    expect(result.TELEGRAM_GROUP_POLICY).toBe("open");
    expect(result.TELEGRAM_GROUP_ALLOW_FROM).toBe("group_user1");
    expect(result.TELEGRAM_REQUIRE_MENTION).toBe("true");
  });
});
